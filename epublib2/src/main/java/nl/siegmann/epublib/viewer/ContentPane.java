...
public class ContentPane extends JPanel implements NavigationEventListener,
	...
	private static final Logger log = Logger.getLogger(ContentPane.class.getName());
...
//					|| (keyEvent.getKeyCode() == KeyEvent.VK_DOWN)) {
	...
	public void displayPage(Resource resource, int sectionPos) {
		...
		try {
			...
			log.log(Level.WARNING, "When reading resource " + resource.getId() + "("
	...
	public void hyperlinkUpdate(HyperlinkEvent event) {
        ...
        if (url.getProtocol().toLowerCase().startsWith("http") && !"".equals(url.getHost())) {
            if(launchBrowser(event.getURL())) {
                return;
            }
            log.log(Level.WARNING, "Couldn't launch system web browser.");
        }
		...
		if (resource == null) {
			log.warning("Resource with url " + resourceHref + " not found");
		} else {
   ...
   */
	private static boolean launchBrowser(URL url) {
