...
public class JpegDecoder {
	...
	private final static int IDCT_P[] = { 0, 5, 40, 16, 45, 2, 7, 42, 21, 56, 8, 61, 18, 47, 1, 4, 41, 23, 58, 13, 32, 24, 37, 10, 63, 17, 44, 3, 6, 43, 20,
			57, 15, 34, 29, 48, 53, 26, 39, 9, 60, 19, 46, 22, 59, 12, 33, 31, 50, 55, 25, 36, 11, 62, 14, 35, 28, 49, 52, 27, 38, 30, 51, 54 };
	private final static int table[] = { 0, 1, 5, 6, 14, 15, 27, 28, 2, 4, 7, 13, 16, 26, 29, 42, 3, 8, 12, 17, 25, 30, 41, 43, 9, 11, 18, 24, 31, 40, 44, 53,
	...
	class DefaultPixelArray implements PixelArray {
		...
		public Image getImage() {
			MemoryImageSource mi = new MemoryImageSource(width, height, pix, 0, width);

			return Toolkit.getDefaultToolkit().createImage(mi);
		}
	}
	...
	class HuffmanTable {
		int Lh, // Length
				...
				L[][][] = new int[4][2][16], V[][][][] = new int[4][2][16][200]; // tables
	...
	/*
	 * Build_HuffTab() Parameter: t table ID c table class ( 0 for DC, 1 for AC ) L[i] # of codewords which length is i V[i][j] Huffman Value (length=i) Effect:
	 * build up HuffTab[t][c] using L and V.
	 */

	private void Build_HuffTab(int tab[], int L[], int V[][]) throws Exception {
		int current_table, i, j, n, temp;
		int k;
		temp = 256;
		k = 0;
	...
	/*
	 * HuffmanValue(): return: Huffman Value of table 0xFF?? if it receives a MARKER Parameter: table HuffTab[x][y] (ex) HuffmanValue(HuffTab[1][0],...) temp
	 * temp storage for remainded bits index index to bit of temp in FILE pointer Effect: temp store new remainded bits index change to new index in change to
	 * new position NOTE: Initial by temp=0; index=0; NOTE: (explain temp and index) temp: is always in the form at calling time or returning time | byte 4 |
	 * byte 3 | byte 2 | byte 1 | | 0 | 0 | 00000000 | 00000??? | if not a MARKER ^index=3 (from 0 to 15) 321 NOTE (marker and marker_index): If get a MARKER
	 * from 'in', marker=the low-byte of the MARKER and marker_index=9 If marker_index=9 then index is always > 8, or HuffmanValue() will not be called.
	 */
	...
	/*
	 * output() x, y should be the starting point of MCU when calling output(..) it means output() should set x,y for the next MCU at the end.
	 */
	...
	private void output(PixelArray out) {
		...
		for (int i = 0; i < YV; i++) {
			for (int j = 0; j < YH; j++) {
				...
				for (int l = 0; l < 64; l++) {
					if (x < Xsize && y < Ysize) {
						out.setPixel(x, y, YUV_to_BGR(DU[0][k][l] + 128, DU10[temp_8y + temp_x], DU20[temp_8y + temp_x]));
					}
					x++;
					if ((x % YH) == 0)
						temp_x++;
	...
	/*
	 * decode_MCU() return 0 if correctly decoded 0xFF?? if it sees a MARKER
	 */
	...
	public Image decode(InputStream in) throws Exception {
		DefaultPixelArray defaultPixelArray = new DefaultPixelArray();
		decode(in, defaultPixelArray);
		return defaultPixelArray.getImage();
	}
